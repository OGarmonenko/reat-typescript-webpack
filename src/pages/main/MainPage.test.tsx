...
describe('Test MainPage with initial state', () => {
  ...
  test('httpService is called, if click on button "Delete"', async () => {
    ...
    await waitFor(() => expect(removeRecord).toHaveBeenCalledWith(mockData[0].id));
  });

  test('should be modal window, if get error after get request', async () => {
  ...
  test('should be modal window, if get error after delete request', async () => {
    getRecords.mockResolvedValue(mockData);
  ...
  test('should be modal window, if get error after add request', async () => {
    addRecord.mockRejectedValue(new Error('error'));
    render(componentMainPage);
    const input = screen.getByTestId('customInput');
    fireEvent.change(input, { target: { value: 'test2' } });
    const button = screen.getByText('Add');
